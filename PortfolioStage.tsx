...
/* ------------------------------------------------------------------------
   Scattered rest pose for a card at logical slot k. Uses deterministic
   seed-based randomness so positions are stable across reloads but feel
   organic. Cards are jittered across a coarse grid so they don't overlap
   too much (minimum spacing ≈ 0.7 world units).
   ------------------------------------------------------------------------ */
type RestPose = { x: number; y: number; z: number; scale: number; rotZ: number };

function buildScatteredPoses(count: number, seed: number): RestPose[] {
...
interface CardEntry {
	...
	aspectHW: number;
	// Per-card drag offset (in stack-group LOCAL world units). Added on top of
	// the rest pose so each card can move independently of the others.
	dragOffsetX: number;
	dragOffsetY: number;
	// True while this card is being dragged by the active pointer. While true
	// all secondary motion (ambient float, hover detection re-runs, stack tilt
	// subtraction) is suppressed so the drag write is the ONLY thing moving
	// the card — prevents flicker from competing writes.
	dragging: boolean;
	// Eased 0..1 hover state. 1 ⇒ fully popped (scale +0.04, shadow lifted).
	hoverT: number;
}
...
export default function StackStage(props: StageProps) {
	...
	const restPosesRef = React.useRef<RestPose[]>([]);
	const N_CLAMPED = Math.max(2, Math.min(MAX_CARDS, Math.round(cardCount)));
	const restPoses = React.useMemo(
		() => buildScatteredPoses(N_CLAMPED, stableSeed),
		[N_CLAMPED, stableSeed],
	);
	...
	const dragCardIndexRef = React.useRef<number | null>(null);
	...
	const hoverCardIndexRef = React.useRef<number>(-1);
	...
	const ndcRef = React.useRef<THREE.Vector2>(new THREE.Vector2());
	...
	React.useEffect(() => {
		...
		if (remaining === 0) setAssetReady(true);
		const settleOne = () => {
			remaining -= 1;
			if (remaining <= 0) setAssetReady(true);
		};
	...
	React.useEffect(() => {
		...
		const clientToHit = (
			clientX: number,
			clientY: number,
		): { hitCardIndex: number; localX: number; localY: number } | null => {
			const camera = cameraRef.current;
			const stack = stackGroupRef.current;
			const cards = cardsRef.current;
			if (!camera || !stack) return null;
			...
			for (const c of cards) meshes.push(c.card);
			...
			if (hits.length > 0) {
				const h = hits[0]!;
				const idx = (h.object as THREE.Mesh).userData.cardIndex;
				if (typeof idx === 'number') hitCardIndex = idx;
			}
			...
			if (t < 0) return { hitCardIndex, localX: 0, localY: 0 };
		...
		const onPointerDown = (event: PointerEvent) => {
			const hit = clientToHit(event.clientX, event.clientY);
			if (!hit || hit.hitCardIndex < 0) return;
		...
		const onPointerLeave = (event: PointerEvent) => {
			...
			if (event && event.pointerId === dragPointerIdRef.current) return;
	...
	useHeroAnimationFrame(rootRef, ({ delta }) => {
		...
		for (let i = 0; i < N; i++) {
			...
			if (card.dragging) {
				...
				if (card.hoverT < 0.0005 && targetHover === 0) card.hoverT = 0;
			}
			...
			const shadowMat = card.shadow.material as THREE.MeshBasicMaterial;
			...
			card.shadow.position.set(
				0.025 * cardW + card.hoverT * 0.02 * cardW,
				-0.02 * cardW - card.hoverT * 0.04 * cardW,
				-0.015,
			);
		}
